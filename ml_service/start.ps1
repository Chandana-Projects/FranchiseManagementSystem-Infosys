...
if (-not (Test-Path $modelFile)) {
    Write-Host "[*] Training ML models (first time, takes ~30 seconds)..." -ForegroundColor Yellow
    $env:PYTHONIOENCODING = "utf-8"
    python models/trainer.py
    Write-Host "   [OK] Models trained and saved" -ForegroundColor Green
} else {
    Write-Host "[OK] ML models already trained -- skipping training" -ForegroundColor Green
}

# 5. Start FastAPI server
Write-Host "Starting ML microservice on http://localhost:8000 ..." -ForegroundColor Cyan
